...
public class StressNoStressData {
    ...
    private static void adjustData(ArrayList<Double> first, 
        ...
        try {
            if (first.size() > second.size()) {
                ...
                while (first.size() != second.size()) {
                    ...
                    Double newData = (first.get(first.size() - 1) + 
                            first.get(first.size() - 2)) / 2;
                ...
                while (second.size() != first.size()) {
                    Double newData = (second.get(second.size() - 1) + 
                            second.get(second.size() - 2)) / 2;
