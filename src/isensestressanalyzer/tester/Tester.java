...
public class Tester 
{
    ...
    public void createARFFFileForSearchTask() {
    	...
    	try {
	    	...
	    	File fileVertical = new File(SearchAnalyzer.OUTPUT_FOLDER + this.getName() + File.separator + "VerticalScroll.arff"), 
	    			fileHorizontal = new File(SearchAnalyzer.OUTPUT_FOLDER + this.getName() + File.separator + "HorizontalScroll.arff"),
	    			fileVerticalHorizontal = new File(SearchAnalyzer.OUTPUT_FOLDER + this.getName() + File.separator + "VerticalHorizontalScroll.arff");
	    	...
	    	BufferedWriter writerVertical = new BufferedWriter(new FileWriter(fileVertical)), 
	    			writerHorizontal = new BufferedWriter(new FileWriter(fileHorizontal)), 
	    			writerVerticalHorizontal = new BufferedWriter(new FileWriter(fileVerticalHorizontal));
    ...
    public void createARFFFileForWriteTask() {
    	try {
    		...
    		File fileOutput = new File(Analyzer.OUTPUT_FOLDER + this.getName() + 
    				File.separator + "write.arff");
	...
	public static void createGlobalARFF(ArrayList<Tester> mListTester) {
		...
		try {
			...
			File globalFileVertical = new File(SearchAnalyzer.OUTPUT_FOLDER + "globalModelVertical.arff"),
					globalFileHorizontal = new File(SearchAnalyzer.OUTPUT_FOLDER + "globalModelHorizontal.arff"),
					globalFileVerticalHorizontal = new File(SearchAnalyzer.OUTPUT_FOLDER + "gloablModelVerticalHorizontal.arff"), 
					globalFileWrite = new File(Analyzer.OUTPUT_FOLDER + "globalModelWrite.arff");
			...
			BufferedWriter writerVertical = new BufferedWriter(new FileWriter(globalFileVertical)),
					writerHorizontal = new BufferedWriter(new FileWriter(globalFileHorizontal)), 
					writerVerticalHorizontal = new BufferedWriter(new FileWriter(globalFileVerticalHorizontal)), 
	 ...
	 */
	private static void createPreambleARFFFile(BufferedWriter writer, boolean write, 
			boolean vertical, boolean horizontal) {
		
		try {
			writer.write("@RELATION stress"); writer.newLine();
			if (vertical) {
				ArrayList<String> attributesVertical = Search.getListARFFAttributesNames(true);
		    ...
		    if (horizontal) {
		    	ArrayList<String> attributesHorizontal = Search.getListARFFAttributesNames(false);
	 ...
	 */
	private static void addScrollsToFile(Tester tester, BufferedWriter writer, boolean vertical, 
			boolean horizontal) {
		
		try {
			
			ArrayList<Search> searches = tester.getSearchExercisesForProtocol(ISenseStressAnalyzer.protocols[0]);
	    	
	    	for (Search exercise: searches) {
	    		ArrayList<ArrayList<Double>> attribuesForAllVerticalScrolls = 
	    				exercise.ARFFAttributesForScroll(true, tester.getPhoneSettings());
	    		ArrayList<ArrayList<Double>> attributesForAllHorizontalScrolls = 
	    				exercise.ARFFAttributesForScroll(false, tester.getPhoneSettings());
	    		
	    		if (vertical) {
		    		for (ArrayList<Double> attributesValues: attribuesForAllVerticalScrolls) {
		    			
		    			if (attributesValues.size() == Search.getListARFFAttributesNames(true).size()) {
			    			...
			    			if (horizontal) {
			    				finalListAttributes += 
			    						Tester.createStringMissingAttributes(Search.getListARFFAttributesNames(false).size());
			    			}
	    		...
	    		if (horizontal) {
		    		for (ArrayList<Double> attributesValues: attributesForAllHorizontalScrolls) {
		    			
		    			if (attributesValues.size() == Search.getListARFFAttributesNames(false).size()) {
		    				
		    				String finalListAttributes = "";
		    				if (vertical) {
		    					finalListAttributes = 
		    							Tester.createStringMissingAttributes(Search.getListARFFAttributesNames(true).size());
		    				}
	...
	private static void addDigitsToFile(Tester tester, BufferedWriter writer) {
		...
		try {
			...
			ArrayList<Write> allWrites = tester.getWriteExercisesForProtocol(ISenseStressAnalyzer.protocols[0]);
			...
			for (Write writeExc: allWrites) {
				...
				for (ArrayList<Double> attributes: attributesForDigits) {
					...
					if (attributes.size() == Write.getARFFListAttributesNames().size()) {
